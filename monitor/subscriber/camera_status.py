...
class CameraStatusMonitorSubscriber(QThread):
    
    status_update_signal = pyqtSignal(str) # connection status message update signal

    def __init__(self, context:zmq.Context, connection:str, topic:str):
        super().__init__()

        self.__console = ConsoleLogger.get_logger()   # console logger
        self.__console.info(f"Camera Status Monitor Connection : {connection} (topic:{topic})")
        ...
        self.__console.info("* Start Camera Status Monitor Subscriber")
        ...
        while not self.isInterruptionRequested():
            try:
                ...
                if self.__socket in events:
                    topic, status_data = self.__socket.recv_multipart()
                    if topic.decode() == self.__topic:
                        self.status_update_signal.emit(status_data.decode()) # emit
    ...
    def socket_monitor(self, socket:zmq.SyncSocket):
        ...
        try:
            ...
            while not self._monitor_thread_stop_event.is_set():
                ...
                # self.status_update_signal.emit(msg) # emit message to signal

...
#ifndef FLAME_TEST_IMAGE_PUSHER_HPP_INCLUDED
#define FLAME_TEST_IMAGE_PUSHER_HPP_INCLUDED
...
class test_image_pusher : public flame::component::object {
    public:
        test_image_pusher() = default;
        virtual ~test_image_pusher() = default;
    ...
    private:
        /* for device handle */
        map<int, int> _device_map; // (camera id, instance)
    ...
    private:
        /* subtasks */
        void _camera_control_task(int camera_id); /* camera control */
        void _image_stream_task(int camera_id, json parameters); /* image capture & flush in pipeline */ 
